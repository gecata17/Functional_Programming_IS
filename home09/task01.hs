...
main = do
    ...
    print $ getLevel numberBTree 3 == [96, 21]
...
traverseBFS t = helper 0
 where
     helper k
      | nodesAtLvlK == [] = []
      | otherwise = nodesAtLvlK ++ helper (k + 1)
       where
...
traverseDFS Nil = []
traverseDFS (Node value left right) = (traverseDFS left) ++ [value] ++ (traverseDFS right) -- (traverseDFS left) ++ (value : traverseDFS right)
-- [5, 12, 1, 96, 0]
-- Node 96 Nil Nil
-- [] ++ [96] ++ [] => [96]
